 ...
 * UI controller.
 */
class PractitionerUIController extends EntityDefaultUIController {
  /**
  ...
  public function hook_menu() {
    ...
    // Change the overview menu type for the list of Practitioner.
    $items[$this->path]['type'] = MENU_LOCAL_TASK;

    // Replaces default entity title.
    $items[$this->path]['title'] = 'Practitioner';

    // Extend the 'edit' path.
    $items['practitioner/' . $wildcard . '/edit'] = array(
      'title callback' => 'practitioner_page_title',
    ...
    $items[$this->path . '/add'] = array(
      ...
      'title arguments' => array('add', $this->entityType, 'practitioner'),
      'page callback' => 'entity_ui_get_form',
      'page arguments' => array($this->entityType, practitioner_create(), 'add'),
      'access arguments' => array('edit any practitioner profile'),
    );

    $items['admin/content/practitioner/' . $wildcard] = array(
      'title callback' => 'practitioner_page_title',
      'title arguments' => array(3),
      'page callback' => 'practitioner_page_view',
      'page arguments' => array(3),
      'access callback' => 'entity_access',
      'access arguments' => array('view', $this->entityType),
    );
  
    $items['admin/structure/practitioner'] = array(
      'title' => 'Practitioner',
      'type' => MENU_NORMAL_ITEM,
      'access arguments' => array('administer practitioner'),
      'page callback' => 'practitioner_bundle_config',
    );
  ...
  public function overviewTable($conditions = array()) {
    ...
    foreach ($entities as $entity) {
      ...
      $additional_cols = [
        'type' => $practitioner->type(FALSE),
        'email' => $practitioner->email,
        'user' => l($practitioner->user()->name, 'user/' . $practitioner->user()->uid),
  ...
  protected function overviewTableRow($conditions, $id, $entity, $additional_cols = array()) {
    ...
    if (entity_has_status($this->entityType, $entity, ENTITY_FIXED)) {
      ...
      if($entity->status) {
        $row[] = l(t('disable'), $this->path . '/manage/' . $id . '/disable', array('query' => drupal_get_destination()));
      }
      else {
        $row[] = l(t('enable'), $this->path . '/manage/' . $id . '/enable', array('query' => drupal_get_destination()));
      }
